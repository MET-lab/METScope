                                 ...
                                 AudioBufferList 			*ioData)
{
    ...
    if (player->bufferSizeFrames == inNumberFrames) {
        ...
        pthread_mutex_lock(&player->currentBufferMutex);
        free(player->currentBuffer);
        player->currentBuffer = (Float32 *)malloc(player->bufferSizeFrames * sizeof(Float32));
        pthread_mutex_unlock(&player->currentBufferMutex);
    }
    ...
    pthread_mutex_lock(&player->currentBufferMutex);
    memcpy(player->currentBuffer, procBuffer, player->bufferSizeFrames * sizeof(Float32));
    pthread_mutex_unlock(&player->currentBufferMutex);
    
    /* Update the long-scale buffer */
    pthread_mutex_lock(&player->longScaleBufferMutex);
    for (int i = 0; i < player->longScaleBufferLength-inNumberFrames; i++)
        player->longScaleBuffer[i] = player->longScaleBuffer[i+inNumberFrames];
    
    for (int i = 0; i < inNumberFrames; i++) {
        player->longScaleBuffer[player->longScaleBufferLength-(inNumberFrames-i)] = procBuffer[i];
    }
    pthread_mutex_unlock(&player->longScaleBufferMutex);
...
- (id)init {
    ...
    if (self) {
        ...
        pthread_mutex_init(&player.currentBufferMutex,  NULL);
        pthread_mutex_init(&player.longScaleBufferMutex, NULL);
        player.currentBuffer  = (Float32 *)malloc(player.bufferSizeFrames * sizeof(Float32));
        player.longScaleBufferLength = kMaxPlotBufferTime * kAudioSampleRate;
        player.longScaleBuffer = (Float32 *)malloc(player.longScaleBufferLength * sizeof(Float32));
        [self setUpAUGraph];
    }
...
- (UInt32)getLongScaleBufferLength {
    return player.longScaleBufferLength;
}
...
- (void)updateCurrentBuffer:(Float32 *)inBuffer {
    
    pthread_mutex_lock(&player.currentBufferMutex);
    free(player.currentBuffer);
    player.currentBuffer = (Float32 *)malloc(player.bufferSizeFrames * sizeof(Float32));
    memcpy(player.currentBuffer, inBuffer, player.bufferSizeFrames * sizeof(Float32));
    pthread_mutex_unlock(&player.currentBufferMutex);
}
- (void)updateLongScaleBuffer:(Float32 *)inBuffer {
    
    pthread_mutex_lock(&player.longScaleBufferMutex);
    free(player.longScaleBuffer);
    player.longScaleBuffer = (Float32 *)malloc(player.longScaleBufferLength * sizeof(Float32));
    memcpy(player.longScaleBuffer, inBuffer, player.longScaleBufferLength * sizeof(Float32));
    pthread_mutex_unlock(&player.longScaleBufferMutex);
}
- (void)getCurrentBuffer:(Float32 *)outBuffer {
    
    pthread_mutex_lock(&player.currentBufferMutex);
    memcpy(outBuffer, player.currentBuffer, player.bufferSizeFrames * sizeof(Float32));
    pthread_mutex_unlock(&player.currentBufferMutex);
}
- (void)getLongScaleBuffer:(Float32 *)outBuffer {
    
    pthread_mutex_lock(&player.longScaleBufferMutex);
    memcpy(outBuffer, player.longScaleBuffer, player.longScaleBufferLength * sizeof(Float32));
    pthread_mutex_unlock(&player.longScaleBufferMutex);
}
