...
- (void)drawRect:(CGRect)rect {
    ...
    if (fillMode) {
        ...
        for (int i = 1; i < resolution-1; i++) {
            ...
            current.y += 2*(parent.originPixel.y - previous.y);
            [path addLineToPoint:current];
...
- (id)initWithParentView:(METScopeView *)parentView {
    ...
    if (self) {
        ...
        fontSize = 11;
        labelAttributes = @{NSFontAttributeName:[UIFont fontWithName:@"Arial" size:fontSize],
...
- (void)setFontSize:(int)pSize {
    labelAttributes = @{NSFontAttributeName:[UIFont fontWithName:@"Arial" size:pSize],
                        NSParagraphStyleAttributeName:[NSMutableParagraphStyle defaultParagraphStyle],
                        NSForegroundColorAttributeName:color};
}
...
@synthesize minimumPlotRange;
@synthesize maximumPlotRange;
...
- (void)setDefaults {
    ...
    minimumPlotRange.x = 0;
    minimumPlotRange.y = 0;
    maximumPlotRange.x = INFINITY;
    maximumPlotRange.y = INFINITY;
...
- (void)setAxisColor:(UIColor *)color {
    [axesSubview setColor:color];
}
- (void)setGridColor:(UIColor *)color {
    [gridSubview setColor:color];
}
- (void)setLabelColor:(UIColor *)color {
    [labelsSubview setColor:color];
}
- (void)setLabelSize:(int)size {
    [labelsSubview setFontSize:size];
}
...
- (void)handlePinch:(UIPinchGestureRecognizer *)sender {
    ...
    /* Otherwise, expand/contract the plot bounds */
        ...
        /* Rescale if we're within the hard limit and not at min/max plot ranges */
        if (xPinchZoomEnabled && newXMin > minPlotMin.x && newXMax < maxPlotMax.x && (newXMax-newXMin) > minimumPlotRange.x && (newXMax-newXMin) < maximumPlotRange.x)
            [self setVisibleXLim:newXMin max:newXMax];
        if (yPinchZoomEnabled && newYMin > minPlotMin.y && newYMax < maxPlotMax.y && (newYMax-newYMin) > minimumPlotRange.y && (newYMax-newYMin) < maximumPlotRange.y)
